avaliar_modelo <- function(
  ...
  # A estrutura abaixo define uma função auxiliar responsável por calcular
  # métricas de classificação para um limiar específico. São derivados
  # acurácia, sensibilidade, especificidade, precisão, F1 e o índice de
  # Youden, a partir da matriz de confusão. Essa abordagem concentra em um
  # único ponto a lógica de avaliação, facilitando a aplicação sistemática
  # a uma sequência de limiares.
  ...
  tabela_limiares <- do.call(
    ...
    lapply(
      limiares,
      function(l) calcula_metricas_limiar(y_tr_true, prob_tr, l)
    )
  )
  ...
  # classes discretas para treino e teste. A função metricas_classificacao,
  # definida em outro módulo do projeto, é reutilizada para consolidar as
  # métricas relevantes de desempenho, garantindo consistência na forma de
  # avaliação em todo o pipeline.
  pred_tr <- factor(
    ifelse(prob_tr > limiar_escolhido, 1, 0),
    levels = c(0, 1)
  )
  metr_treino <- metricas_classificacao(y_tr_true, pred_tr)
  ...
  metr_teste <- metricas_classificacao(y_te_true, pred_te)
...
validacao_cruzada_logistica <- function(
  ...
  # A função auxiliar abaixo resume a lógica de cálculo de métricas para
  # um conjunto de probabilidades e um limiar pré-definido. Ela repete a
  # mesma estrutura usada na avaliação global, o que assegura coerência
  # entre as métricas reportadas na validação cruzada e na avaliação
  # final do modelo.
  ...
  # calculadas no limiar operacional fixado, e o AUC é computado por
  # meio do pacote pROC, fornecendo uma medida global de discriminação.
